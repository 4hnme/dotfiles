...
hook global WinSetOption filetype=odin %{
    remove-hooks window odin-.+ # why?
    require-module odin


    set-option window static_words %opt{odin_static_words}
    # doesn't fucking work, but at least it also doesn't overwrite the tags file
    # set-option window ctagscmd '~/thirdparty/odin-tags/bin/odin-tags -o tags ~/thirdparty/odin/base/core ~/thirdparty/odin/vendor .'
...
add-highlighter shared/odin/code/operators regex (\+|-|/|\*|\^|&|\||!|\?|>|<|%|:|::|:=|~|=|!=|==|<=|>=|\+=|-=|\*=|/=|%=|&=|\|=|\^=|>>=|<<=|>>>=)=? 0:operator
add-highlighter shared/odin/code/function regex _?[a-zA-Z]\w*\s*(?=\() 0:function
