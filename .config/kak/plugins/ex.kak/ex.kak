...
define-command -hidden ls_impl -params 1 %{
  evaluate-commands -save-regs '"' %{
    unset-option buffer modelinefmt
    fifo -name '*ls*' -- %opt{ls_command} %opt{ls_args} -- %arg{1}
    set-option buffer ls_working_directory %sh{
      realpath -- "$1"
    }
    set buffer modelinefmt "{StatusLineValue}%opt{ls_working_directory}/%val{selection}{StatusLine}|%opt{modelinefmt}"
  }
}

complete-command ls file

set-face global LsDirectory +b@function
...
hook global BufSetOption filetype=ls %{
  ...
  # map -docstring 'jump to files or directories' buffer normal <ret> ':jump_to_files_or_directories_and_close_ls_buffer<ret>'
  ...
  map -docstring 'close ls buffer' buffer normal <esc> ':delete-buffer *ls*<ret>'
  # map -docstring 'jump to files or directories' buffer normal <space> ':jump_to_files_or_directories<ret>'
  map -docstring 'jump to parent directory' buffer normal <backspace> '/\.\.<ret>:jump_to_files_or_directories<ret>'
}
